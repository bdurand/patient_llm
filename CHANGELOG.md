...
- Added `PatientLLM::Agent`, a declarative base class bundling provider, model, system, generation settings, tools (schema and handler together as instance methods), structured output schema, and completion handling in one class. Call with `MyAgent.ask(message, context: {...})`, continue persisted conversations with `MyAgent.continue(state, message)`, and run inline for consoles and tests with `MyAgent.ask!`. Responses are handled by `completed(response)`, `failed(failure)`, and `tool_round(response)` hooks: successes receive an `Agent::Response` and failures an `Agent::Failure`, each bundling the whole invocation — the context passed to `ask`/`continue`, the HTTP exchange (`http_response`/`http_request_id`, nil for non-HTTP errors), the session, and the response text/structured output or error. Both objects support `[]` as a shorthand for reading a context value (`response[:trip_id]`). Tool methods can opt into the context by declaring a `context:` keyword.
- Added a `callback:` option to `PatientLLM::Agent.ask` (and `continue`/`ask!`) naming a class to receive the `completed`, `failed`, and `tool_round` hooks instead of the agent, so one agent's provider, tools, and output schema can be reused with different completion handling. The class is instantiated per invocation and may implement any subset of the hooks; the rest fall back to the agent's own. Only the class name travels through the queue, so it must be a named, resolvable class.
- Added an `extra` declaration to `PatientLLM::Agent` for provider-specific session data (e.g. the Bedrock Converse `guardrail_config`), set on every session the agent builds. It is inherited by subclasses, removable with an explicit `nil`, and overridable per request with `ask(extra: {...})`; `continue` re-applies the agent's current value to restored sessions.
