...
RSpec.describe PatientLLM::Callback do
  ...
  describe "auto tool loop" do
    ...
    it "restores the processor override when re-asking after tool execution" do
      args = callback_args_with_tools.merge(request_options: {"processor" => "llm"})
      ...
      begin
        ...
        expect(captured).not_to be_nil
        expect(captured[:request].processor).to eq("llm")
      ensure
        PatientHttp.unregister_handler
      end
    end
